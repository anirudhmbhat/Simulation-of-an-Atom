...
int n = 5,x,y,z,rot2,tflag = 0,scaleflag = 0,xlight = 2,ylight = 2,zlight = 2,frustum = 0,rotate = 0;
int lightflag = 1,randomColorFlag = 0,count,axesflag = 0,rotx,roty,menuredrawflag = 0;		//Scaleflag 2 for only electrons and 1 for the entire atom
...
//float ele[][3] = {{4,0,0}, {0,4.5,0}, {-5,5,5}, {5.5,5.5,5.5}, {6,0,0}, {0,6.5,0}, {-7,7,7}, {7.5,7.5,7.5}};
float ele[][3] = {{4,0,0}, {0,4.5,0}, {-2.8868,2.8868,2.8868}, {3.1755,3.1755,3.1755}, {6,0,0}, {0,6.5,0}, {-4.0415,4.0415,4.0415}, {4.3301,4.3301,4.3301}};
...
char num[][11] = {{"Electron 1"},{"Electron 2"},{"Electron 3"},{"Electron 4"},{"Electron 5"},{"Electron 6"},{"Electron 7"},{"Electron 8"}};
...
void processPositionLight(int option)
{
	switch(option)
	{
		case Y_AXIS:	light_position[0] = 0;light_position[1] = 30.0;light_position[2] = 0;break;
		case Z_AXIS:	light_position[0] = 0;light_position[1] = 0.0;light_position[2] = 30.0;break;
		case X_AXIS:	light_position[0] = 30.0;light_position[1] = 0.0;light_position[2] = 0;break;
	}
}
...
void reshape(int w,int h)
{
	...
	if(w <= h)
		...
		glOrtho(-10.0 * (float) w / (float) h, 10.0 * (float) w / (float) h,-10,10,-30,30);
	}
	else
	{
		if(w <= h)
		glFrustum(-5,5,-5.0 * (float) h / (float) w,5.0 * (float) h / (float) w,1.0,200.0 * (float) h/ (float) w);
	else
		glFrustum(-5.0 * (float)w / (float) h,5.0 * (float) w / (float) h,-5.0, 5.0,1.0,200.0 * (float) w/ (float) h);	
	}
...
void myinit()
{
	...
	{
	if(w1 <= h1)
		glOrtho(-10,10,-10.0 * (float)h1/(float)w1,10.0 * (float) h1/ (float) w1,-30,30);
	else
		glOrtho(-10.0 * (float) w1 / (float) h1, 10.0 * (float) w1 / (float) h1,-10,10,-30,30);
	}
	else
	{
		if(w1 <= h1)
		glFrustum(-5,5,-5.0 * (float) h1 / (float) w1,5.0 * (float) h1 / (float) w1,1.0,200.0);
	else
		glFrustum(-5.0 * (float)w1 / (float) h1,5.0 * (float) w1 / (float) h1,-5.0, 5.0,1.0,200.0);	
	}
	}
	glMatrixMode(GL_MODELVIEW);	
...
void first()
{
	...
	glColor3f(0,0.5,1);
	strokeString(50,100,0.17,0.17,"1 . Set the number of electrons using the menu",2);
	strokeString(50,50,0.17,0.17,"2 . Press (Enter) to start the simulation",2);
	strokeString(650,200,0.15,0.15,"By:",2);
	glColor3f(0,0.5,1);
	strokeString(650,160,0.18,0.18,"Anirudh.M.Bhat",2);
	strokeString(650,30,0.2,0.2,"Girija.J(Associate Professor)",2);
	glColor3f(1,0,0);
	strokeString(650,120,0.18,0.18,"1BI13CS016",2);
